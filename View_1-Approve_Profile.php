...
class Approve_Profile extends Default_Profile  {
   ...
   public $custom_type = 'view_1';
    ...
    public function __construct() {
        ...
        // set custom class for hide the checkbox when profile_incomplete meta field should be yes     
       add_filter( 'post_class', array($this,'savi_app_pro_filter_post_class') );  
    ...
    public function savi_app_pro_save_postdata( $post_id ) {
     ...
     function savi_app_pro_filter_post_class( $classes ) {
  
       global $post;
       $profile_incomplete = get_post_meta(get_the_ID(),'profile_incomplete',true);
		 if( $post->post_type == 'view_1' && $profile_incomplete =="yes" ){
 
              $my_post_class = 'profile_incomplete';
